        ...
        async def _async_scan():
            def detection_callback(device, advertisement_data):
                try:
                    ...
                    is_medflo = name.startswith("MEDFLO-") or name.startswith("MFL-") or name.startswith("MFS-") or mac.startswith("F44EFD") or mac.startswith("A100")
                    ...
                    if has_shared_mfg:
                        payload = mfg_dict[0xFFFF]
                        is_new_fw = name.startswith("MFL-") or name.startswith("MFS-")
                        # Auto-detect Company ID prefix (0xFF 0xFF) if included in payload
                        offset = 2 if (is_new_fw and len(payload) >= 6 and payload[0] == 0xFF and payload[1] == 0xFF) else 0
                        if is_new_fw:
                ...
                while True:
                    try:
                        async with BleakScanner() as scanner:
                            print("[BLE Server Engine] Scanner started, listening for BLE advertisements...")
                            async for device, advertisement_data in scanner.advertisement_data():
                                detection_callback(device, advertisement_data)
                    except Exception as inner_ex:
                        print(f"[BLE Server Engine] BleakScanner loop exception: {inner_ex}")
                        await asyncio.sleep(2.0)
            except Exception as ex:
                print(f"[BLE Server Engine] Outer scan error: {ex}")
